...
movie_ids = pd.read_csv("data/movie_ids.csv")["Unnamed: 0"]
...
def ibcf(newuser):
    ...
    if valid_predictions_count >= num_top_movies:
        top_rated_indices = sorted_indices[:10]
        top_movies = list(movie_ids[top_rated_indices])
...
# # data = pd.read_csv("data/Rmat.csv").iloc[:1000]
# movie_ids = pd.read_csv("data/movie_ids.csv")["Unnamed: 0"]
# print(list(movie_ids[[1, 4]]))
# # movie_names = []
...
#         top_movies = movie_ids[top_rated_indices].tolist()
#         top_predictions = predictions[top_rated_indices].tolist()
#         return top_movies, top_predictions
